...
const SearchAndFilters = ({ 
  ...
  selectedCategory, 
  setSelectedCategory,
  selectedSaleType, // 'mayor' o 'menor'
  setSelectedSaleType,
  categories 
}) => {
  // Categorías que se muestran en "Por Mayor"
  const mayorCategories = ['Rebozados', 'Cajones', 'Pescados', 'Ofertas'];
  
  // Debug: Ver qué categorías tienes
  console.log('Todas las categorías:', categories);
  console.log('Categorías filtradas para mayor:', categories.filter(cat => mayorCategories.includes(cat.name)));
  
  // Filtrar categorías según el tipo de venta seleccionado
  const getFilteredCategories = () => {
    if (selectedSaleType === 'mayor') {
      return categories.filter(cat => mayorCategories.includes(cat.name));
    }
    return categories; // Para "menor" se muestran todas las categorías
  };
  ...
  return (
    <div className="mb-12 space-y-6 max-w-4xl mx-auto">
      ...
      {/* Filtros Por Mayor / Por Menor */}
      <div className="flex justify-center space-x-4">
        <button
          onClick={() => {
            setSelectedSaleType('mayor');
            setSelectedCategory('all'); // Reset category when changing sale type
          }}
